...
``lemaitre.initial_data`` (this repo, LM-initial-data), and later
``lemaitre.early_inspiral``, ``lemaitre.ringdown``, ``lemaitre.artwork`` from
their sibling repos.
...
``lemaitre/__init__.py``.  While LM-initial-data is the only sibling it lives
here; when a second sibling repo is created, factor this file into a dedicated
tiny ``lemaitre-core`` distribution that every sibling depends on.
"""
