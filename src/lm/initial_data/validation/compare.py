"""B1 Step 4 — LM-initial-data vs TwoPunctures comparison + figures (v)/(vi).

Produces, for an equal-mass head-on slice:
  * psi-on-shared-points agreement vs LM-initial-data resolution (LM-initial-data -> TP as N grows);
  * an ADM table (total ADM mass, individual puncture ADM masses, ADM linear
    momentum, angular momentum) LM-initial-data vs TwoPunctures;
...
The external oracle is invoked only through :mod:`lm.initial_data.validation.twopunctures`.
"""
...
# --------------------------------------------------------------------------
# Step 4a — psi on shared points: LM-initial-data converges to TwoPunctures
# --------------------------------------------------------------------------
def _query_points(b):
...
def psi_agreement(b, m_A, m_B, P, lm_initial_data_grids, tp_res=(64, 64, 4)):
    """max|psi_LM-initial-data - psi_TP| at shared points, vs LM-initial-data grid resolution."""
    rho, z = _query_points(b)
    res = tp.solve_lm_initial_data_points(b, m_A, m_B, P, rho, z,
                                  nA=tp_res[0], nB=tp_res[1], nphi=tp_res[2])
    rows = []
    for (Na, Nb) in lm_initial_data_grids:
...
# --------------------------------------------------------------------------
def adm_table(b, m_A, m_B, P, lm_initial_data_grid=(64, 44), tp_res=(64, 64, 4)):
    prob = sa.make_problem(Na=lm_initial_data_grid[0], Nb=lm_initial_data_grid[1], P=P)
    sl = sa.Slice(b, m_A, m_B)
    U, info = sa.newton_solve(prob, sl, tol=1e-12, max_iter=30)
    ...
    return {
        ...
        "M_ADM": dict(lm_initial_data=adm.adm_mass_spectral(prob, U, sl),
                      tp=res.E),
        "M_A": dict(lm_initial_data=adm.puncture_adm_mass(prob, U, sl, "A"),
                    tp=res.mp_adm),
        "M_B": dict(lm_initial_data=adm.puncture_adm_mass(prob, U, sl, "B"),
                    tp=res.mm_adm),
        "P_A_z": dict(lm_initial_data=adm.by_momentum_gauss(b, P, "A"), tp=-P),
        "P_B_z": dict(lm_initial_data=adm.by_momentum_gauss(b, P, "B"), tp=+P),
        "P_total_z": dict(lm_initial_data=adm.adm_linear_momentum_total(b, P),
                          tp=0.0),
        "J": dict(lm_initial_data=0.0, tp=float(np.max(np.abs(res.J)))),
    }
...
# --------------------------------------------------------------------------
def constraint_curve(b, m_A, m_B, P, Ns, L, lm_initial_data_grid=(52, 36),
                     tp_baseline_N=None, tp_res=(48, 48, 4), r_excl=1.0):
    """FD Hamiltonian + momentum constraint (L2-RMS, bulk) vs grid spacing h.

    Returns (rows, tp_rows): rows = [(N, h, H_L2, M_L2)] for LM-initial-data ID;
...
def _fig_agreement(rows, table, path):
    ...
    ax.set(xlabel="LM-initial-data grid $N_A\\times N_B$",
           ylabel=r"$\max|\psi_{\rm LM-initial-data}-\psi_{\rm TwoPunctures}|$",
           title="(v) LM-initial-data $\\to$ TwoPunctures agreement on shared points")
    ax.grid(True, which="both", alpha=0.3)

    # ADM table panel
    axt.axis("off")
    lines = [["quantity", "LM-initial-data", "TwoPunctures", "rel.diff"]]
    for key in ("M_ADM", "M_A", "M_B"):
        p, t = table[key]["lm_initial_data"], table[key]["tp"]
...
def main(outdir=None, b=3.0, m_A=0.5, m_B=0.5, P=0.5,
         lm_initial_data_grids=((28, 20), (36, 24), (44, 30), (52, 36), (64, 44)),
    ...
    rows, _ = psi_agreement(b, m_A, m_B, P, lm_initial_data_grids)
    ...
    if verbose:
        print("=== (v) psi-on-shared-points agreement (LM-initial-data -> TwoPunctures) ===")
        for Na, Nb, d, rn in rows:
            print(f"  {Na}x{Nb}: max|dpsi|={d:.3e}  (||R||={rn:.1e})")
        print("=== ADM / quasi-local table ===")
        for k, v in table.items():
            if isinstance(v, dict):
                print(f"  {k:10s}: LM-initial-data={v['lm_initial_data']:.10g}  TP={v['tp']:.10g}")
        print("=== (vi) constraint violation vs h (LM-initial-data ID) ===")
