...
``~/.cache/bbhfm/parasol_tp_oracle/build.sh`` or set ``PARASOL_TP_BIN``), so the
...
def test_psi_agreement_on_shared_points(parasol_solve):
    """psi agrees with TwoPunctures at shared points to the two-centre floor."""
    prob, U, sl = parasol_solve
...
def test_total_adm_mass_agreement(parasol_solve):
    prob, U, sl = parasol_solve
...
def test_individual_puncture_mass_agreement(parasol_solve):
    prob, U, sl = parasol_solve
    res = tp.solve_tp(B, MA, MB, P, np.array([[B, 0, 0]]), nA=64, nB=64, nphi=4)
...
def test_no_angular_momentum_head_on(parasol_solve):
    ...
    # PARASOL convention check: per-puncture linear momenta are exactly +-P
