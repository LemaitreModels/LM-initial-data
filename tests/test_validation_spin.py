...
(``par_S_plus/minus``), so PARASOL's spinning ψ and the angular momentum J can
...
def test_conventions_spin_mapping():
    ...
    par = conventions.parasol_to_tp(B, MA, MB, P, S_A=SA, S_B=SB)
    assert par.par_S_plus == (SA, 0.0, 0.0)
    assert par.par_S_minus == (SB, 0.0, 0.0)
    # no-spin default still maps to zero spin (B1 unchanged)
    assert conventions.parasol_to_tp(B, MA, MB, P).par_S_plus == (0.0, 0.0, 0.0)
...
def test_spin_axisymmetry_nphi():
    ...
    r4 = tp.solve_parasol_points(B, MA, MB, P, rho, z, nA=48, nB=48, nphi=4,
                                 S_A=SA, S_B=SB)
    r12 = tp.solve_parasol_points(B, MA, MB, P, rho, z, nA=48, nB=48, nphi=12,
                                  S_A=SA, S_B=SB)
    assert np.max(np.abs(r4.psi - r12.psi)) < 1e-10


def test_spin_psi_agreement_vs_tp():
    """PARASOL's spinning ψ agrees with TwoPunctures at shared meridian points.
    ...
    assert info.residual_norm < 1e-7, f"PARASOL spin solve ‖R‖={info.residual_norm:.2e}"
    rho = np.array([0.30, 0.60, 0.30, 0.90, 2.0]) * B
    z = np.array([1.10, 0.50, -1.10, 0.60, 0.40]) * B
    res = tp.solve_parasol_points(B, MA, MB, P, rho, z, nA=64, nB=64, nphi=4,
                                  S_A=SA, S_B=SB)
    ...
    print(f"\n[P2] spinning ψ: max|ψ_PARASOL - ψ_TP| = {err:.3e}")
