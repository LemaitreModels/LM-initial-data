"""B1 Step 1 — the LM-initial-data <-> TwoPunctures convention map (risk R2).
...
def test_lm_initial_data_to_tp_basic_map():
    tp = cv.lm_initial_data_to_tp(b=3.0, m_A=0.5, m_B=0.5, P=0.5)
    # par_b is the half-separation (= LM-initial-data b); +b puncture carries m_A
    assert tp.par_b == 3.0
...
def test_unequal_mass_assignment():
    tp = cv.lm_initial_data_to_tp(b=2.0, m_A=0.7, m_B=0.3, P=0.1)
    # the +b puncture (LM-initial-data A) must receive m_A, never m_B
...
def test_momentum_sign_antiparallel():
    ...
    tp = cv.lm_initial_data_to_tp(b=4.0, m_A=0.5, m_B=0.5, P=0.37)
...
def test_point_map_roundtrip():
    """LM-initial-data (rho, z) -> TP Cartesian -> (rho, z) is the identity."""
    rng = np.random.default_rng(1)
    for _ in range(20):
        rho = abs(rng.uniform(0, 5))
        z = rng.uniform(-5, 5)
        x_tp, y_tp, z_tp = cv.lm_initial_data_point_to_tp(rho, z)
...
def test_cactus_par_render():
    tp = cv.lm_initial_data_to_tp(b=3.0, m_A=0.6, m_B=0.4, P=0.2)
    s = tp.to_cactus_par()
