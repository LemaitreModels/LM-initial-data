...
% $\mathbf{S}^{A},\mathbf{S}^{B}$ (or their dimensionless counterparts $\boldsymbol{\chi}^{A},\boldsymbol{\chi}^{B}$), while the initial slice is assumed to be conformally flat and maximally sliced.
...
\begin{align}
  ...
  \label{eq:A_S}
\end{align}
where $\mathbf{P}_X$ and $\mathbf{S}^{X}$ are the linear momentum and spin of
puncture $X$, respectively,
$\mathbf{v}_X \equiv \mathbf{S}^{X}\times\mathbf{n}_X$, and
...
\begin{aligned}
  ...
  &+ \frac{18}{r_X^6}\,|\mathbf{S}^{X}\!\times\!\mathbf{n}_X|^2
   + \frac{18}{r_X^5}\,(\mathbf{P}_X\!\times\!\mathbf{S}^{X})\!\cdot\!\mathbf{n}_X ,
\end{aligned}
  \label{eq:A2_self}
\end{equation}
...
Kerr-like \emph{dimensionless} spins $\boldsymbol{\chi}^{X} \equiv \mathbf{S}^{X}/m_X^2$, each a Cartesian
...
Bowen--York spin that enters the analytic source is $\mathbf{S}^{X} = \boldsymbol{\chi}^{X}\, m_X^2$. The
...
with $k\in\mathcal{E}$, write $U\equiv U_{i_1\cdots i_d}(\mathbf{x})$ for the
...
($q=m_A/m_B\ge1$, $A$ the larger hole; $\chi^{X}_{\parallel} \equiv (\mathbf{S}^{X}\!\cdot\!\hat{\mathbf{L}})/m_X^2$
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:walls}%
    ...
    (decades/node) and the inferred nearest real singularity $\theta_\ast$.
    \emph{Left (merger wall, in the separation $b$):} the rate degrades as the
    interval is pushed toward coincidence while $\theta_\ast$ stays pinned at
    $b_\ast\approx0$ --- a hard real singularity fixed at the merger.
    \emph{Right (spin wall, in $\chi$):} the rate degrades only mildly and
    $\chi_\ast$ marches outward, remaining beyond the sampled range --- the
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:joint}%
    ...
    \emph{Left (four-dimensional model, $(b,q,\chi^{A}_{y},\chi^{B}_{y})$):} the bare
...
For a misaligned-spin slice ($|\mathbf{S}^{A}|=0.3$ tilted $56.3^\circ$ off the collision
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:3dval}%
    ...
    \emph{Left:} for a misaligned-spin slice ($\mathbf{S}^{A}=(0.3,0,0.2)$, $b=1.5$,
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:tpval}%
    ...
    spin tilt exactly ($\mathbf{J}=\sum_X \mathbf{S}^{X}$): all magnitudes collapse onto
