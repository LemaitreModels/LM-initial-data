...
\begin{figure*}[t]
  ...
  \caption{\label{fig:tpval}%
    ...
    production box.} Each curve is a median over $100$ quasi-circular
    configurations drawn from the production box by a Latin hypercube, with the
    shaded envelope spanning the sample minimum to maximum; the resolution axis
    ...
    \emph{(a)} The pointwise field difference
    $\|\psi-\psi_{\rm TP}\|_\infty$ falls with resolution while the certified
    Newton--Krylov residual $\|R\|_\infty$ rises. The opposite trends are the
    ...
    \emph{(b)} The integral comparison, $|\Madm-E_{\rm TP}|/E_{\rm TP}$.
    \emph{(c)} The azimuthal Fourier spectrum $|\hat u_m|/|\hat u_0|$ at the
    finest resolution, which sets how many $\phi$ modes the quasi-circular data
    require.}
\end{figure*}
...
Fig.~\ref{fig:tpval}(a). At the coarsest resolution the sample spans
...
which is the statement the band makes. Individual configurations need not:
...
band. At fixed spin the difference grows by roughly two orders of magnitude
...
The certified residual behaves in the opposite direction, its median rising by a
...
The integral comparison, shown in Fig.~\ref{fig:tpval}(b), converges faster than
...
The azimuthal Fourier spectrum in Fig.~\ref{fig:tpval}(c) sets the truncation
$N_\phi$ requires. The quasi-circular data are non-axisymmetric by construction:
...
$9.1\times10^{-5}$. A small number of Fourier modes therefore resolves these
data, and the quasi-circular family exercises the Fourier-in-$\phi$ solver of
...
below the median of the quasi-circular band.
