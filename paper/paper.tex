...
It provides \emph{certified constraint satisfaction}. Every queried parameter is refined by Newton iterations until the constraint residual satisfies $\|R\|_\infty \le 10^{-10}$, ensuring that every returned initial datum satisfies the Einstein constraints to a prescribed tolerance.
The sparse interpolant therefore accelerates the computation but does not replace the underlying elliptic solver.
It provides \emph{exact differentiability}.
...
because the polish can already ensure the correctness.
...
predictor. The decomposition is computed once from the existing corpus, with no new solves.
...
% ===========================================================================
We validate the initial-data solver against the TwoPunctures
code~\cite{AnsorgBruegmannTichy2004}, using its standalone C port from the NRPy
suite compiled against GSL. We compare the quasi-circular data over the whole
production box (Appendix~\ref{sec:validation:qc}) and after interpolation onto
an evolution grid (Appendix~\ref{sec:validation:constraints}). In the
axisymmetric limit the two independently discretized codes agree to near
machine precision.
...
\begin{figure}[t]
  ...
  \caption{\label{fig:tpval}%
    ...
    $\|R\|_\infty$ of Sec.~\ref{sec:solve:newton}, evaluated at the returned
    iterate; the pointwise field difference $\|\psi-\psi_{\rm TP}\|_\infty$; and
    the integral comparison $|\Madm-E_{\rm TP}|/E_{\rm TP}$. The field
    difference falls with resolution while the residual rises: the rise is
    roundoff amplification in unpopulated high-$m$ modes
...
whereas the standard TwoPunctures convention places them on the $x$ axis, we
...
set of $69$ interior probe points, placed clear of both punctures and covering
the full azimuthal range. A sparse probe set underestimates a supremum norm
preferentially where the field is best resolved, because the estimate collapses
whenever the error at the dominant point changes sign; reducing the set to six
points lowers the difference at the finest resolution by a median factor of
$2.7$. The reference solve is converged well below every difference reported:
...
rather than the reference.} At the coarsest resolution the sample spans
...
although $13$ of the $100$ configurations show one non-monotone step somewhere
...
three digits at every resolution. Two parameters control where a configuration
sits within the spread. At fixed spin the difference grows by roughly two orders
of magnitude from $q=1$ to $q=3$, and by a further order from $b=3$ to $b=10$;
nonspinning configurations reach the $10^{-10}$ range. Spin combined with an
unequal mass ratio, not either alone, therefore sets the scale of the
comparison.

The certified residual behaves in the opposite direction
(Fig.~\ref{fig:tpval}). Its median rises by a factor of $27$ along the same
sequence, from $9.7\times10^{-13}$ to $2.6\times10^{-11}$, while remaining below
the certification tolerance $10^{-10}$ for every one of the $100$ configurations
at every resolution; the largest value anywhere in the sample is
$6.5\times10^{-11}$. That the field difference falls while the residual grows
...
configuration reaches $6.2\times10^{-11}$. The two remain comparable, however:
at the finest resolution their medians differ by a factor of $1.1$, and the ADM
mass is the more accurate for only about half of the sample. What decides the
ordering is the separation rather than the spin. At $b=3$ the integral is
between two and twenty-five times the more accurate, while at $b=10$ it is two
to three times the less accurate, its boundary derivative at $A=1$ being read
from an increasingly small $\psi-1$. Both metrics are therefore worth reporting.
...
\begin{figure}[t]
  ...
  \caption{\label{fig:tpspec}%
    ...
    with $m=0$ the normalization and not shown. The expansion of
    ...
    $m=1$--$3$, and is not comparable like-for-like with them. Content above
    $m=4$ folds into the plotted modes rather than appearing beyond them, so the
    panel bounds the high-$m$ amplitudes rather than resolving them; that
    $N_\phi=8$ suffices follows from refinement, reported in the text.}
\end{figure}
...
sample both sit at the percent level, with medians of $1.7\times10^{-2}$ and
$2.2\times10^{-2}$ and maxima of $2.4\times10^{-2}$ and $2.9\times10^{-2}$
respectively. The spectrum then falls by more than two decades, the $m=3$ and
$m=4$ medians being $8.6\times10^{-5}$ and $9.1\times10^{-5}$; the second is the
...
data, as the refinement above establishes independently. The quasi-circular
family therefore exercises the Fourier-in-$\phi$ solver of
Sec.~\ref{sec:solve:abt} on its own.

Two checks that the quasi-circular family cannot supply, because it is never
axisymmetric, complete the picture. First, for a head-on configuration with spin
along the collision axis, all $m\ge1$ modes remain at most $9.2\times10^{-17}$
across mass ratios and spin magnitudes, confirming that the Fourier-in-$\phi$
...
for an equal-mass configuration with $b=3$ and axial momentum $P=0.5$, solved on
the same grid sequence and compared against the same reference, the two
independently discretized spectral codes agree in $\psi$ to $2.6\times10^{-10}$
in the supremum norm and $4.3\times10^{-11}$ in root mean square, and in the
total ADM mass to a relative difference of $5.0\times10^{-11}$; the certified
residual there is $1.2\times10^{-12}$. With no azimuthal structure to limit
either code, this is the most stringent code-to-code reference available for the
solver, some four orders of magnitude below the median of the quasi-circular
sample.
...
The comparison with TwoPunctures is an external cross-check of the numerical
implementation, over the box the parametric models are built on rather than at a
single configuration; constraint satisfaction of the final data is established
independently by the certified residual of Sec.~\ref{sec:polish}.


\subsection{Constraint violation on an evolution grid}
\label{sec:validation:constraints}

As a final validation, we interpolate the spectral initial data onto a uniform
Cartesian grid, as required by a numerical-relativity evolution code, and
evaluate the Hamiltonian and momentum constraints using a generic
finite-difference constraint monitor. The monitor computes the Christoffel
symbols and Ricci tensor directly from finite differences, without making use of
conformal flatness, and therefore provides a test independent of the spectral
discretization used to construct the initial data.

We use the axisymmetric configuration of the preceding section, which carries
the fewest confounds and is where the two initial-data solutions differ least.
Both constraint norms converge at the expected second order with Cartesian grid
spacing $h$, the measured orders being $2.14$ for $\|\mathcal{H}\|_2$ and $2.02$
for $\|\mathcal{M}\|_2$. Repeating the calculation with the conformal factor
obtained from TwoPunctures gives identical constraint violations on the same
Cartesian grids. The measured residuals are therefore set by the
finite-difference truncation error of the constraint monitor rather than by
differences between the two initial-data solutions, and the accuracy of the
spectral initial data lies below the truncation error introduced by the
