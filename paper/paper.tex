...
% --- convenient macros ---
% Physical vectors are set bold italic (\vect); the upright bold \mathbf is
% reserved for the Smolyak multi-index \mathbf{l}, which is an index tuple.
...
residual only through $\Ahat^2$ via $S^{X} = \chi^{X} m_X^2$, so
$\partial R/\partial\chi^{X} = \tfrac{1}{8}(\psiBL+u)^{-7} m_X^2\,\partial \Ahat^2/\partial S^{X}$,
...
but not their cross, leaves approximated; it lowers the joint held-out error
...
pursue it: Sec.~\ref{sec:model:joint} finds that a larger enhanced set degrades the
...
{\color{red} It only requires more memory to store the addional gradient information (see figure memory)}.
...
\todo{production box --- both stated ranges disagree with the canonical definition
in \texttt{pipeline/production\_box.py}. (i) The separation is now
$b\in[3,10]\,M$, i.e.\ $D=2b\in[6,20]\,M$, not $b\in[2,7]\,M$ ``up to $14\,M$'';
the same stale range appears in the caption of Fig.~\ref{fig:polish}. (ii) The spin
half-width is \texttt{CHI\_MAX}$=0.9$, not $0.99$ --- set deliberately to sit
inside the Bowen--York horizon-spin ceiling $\chi_{\rm hor}\approx0.93$, above
which the horizon spin saturates. Which value the shipped corpora were built at is
an author decision, not an editorial fix; see also the physical ranges quoted in
Sec.~\ref{sec:conclusions}.} Each is built at Smolyak level
...
The per-puncture momenta are then $\vect{P}_A=(+P_t,0,-P_r)$,
$\vect{P}_B=(-P_t,0,+P_r)$, with zero net linear momentum and orbital angular
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:peraxis}%
    ...
    are the four-dimensional aligned-spin model. The value-only surrogate
    (``value'') is compared with the gradient-enhanced one
    (``value$+$gradient''), which also matches the certified parameter tangent at
...
\todo{walls rebuild --- ``two walls'' becomes THREE (a mass-ratio wall now sits
between them in Fig.~\ref{fig:walls}); and ``both sweeps are run at equal mass''
cannot cover the $q$ sweep, whose swept axis \emph{is} the mass ratio (it holds
$b=3.5$, spins zero). Rewrite this paragraph once block \texttt{Q\_wall\_q} lands.}
...
receding ones.
\todo{load-bearing claim --- re-derive, do NOT patch. The fastest/slowest framing
of this sentence presumes exactly two walls. The mass-ratio axis is neither the
fastest nor the slowest, so a \emph{pinned} $q_\ast$ would place a second hard wall
on a middling axis and dissolve the dichotomy; a \emph{marching} $q_\ast$ preserves
it but adds a third case. The paragraph for the $q$ wall itself is also still to be
written, and its conclusion is the measurement's to make: $q_\ast$ pinned near
$4.1$ across the three ranges means $q_{\max}=3$ is near a hard limit and the box
cannot widen in $q$; $q_\ast$ marching means a complex pair, and $q_{\max}$ could
rise to cover the asymmetric mass ratios. Under a real-singularity model the
predicted rates are $0.600/0.375/0.170$ at $q_{\max}=3.0/3.5/4.0$.}

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig02_walls.pdf}
  \caption{\label{fig:walls}%
    \textbf{The analyticity walls of the quasi-circular family.} Held-out
    ...
    \emph{Middle (mass ratio $q$):} \stub{pending the $q$-wall measurement}.
    ...
    \todo{on rebuild --- write the middle panel's sentence and, if the measurement
    warrants it, the wall's character into its panel title. Every number here also
    changes: the current curves are the superseded narrow box $b\in[1.5,4]$ in the
    old bare-spin ($S_{Ay},S_{By}$) parameterization.}}
\end{figure}
...
$15{,}713$, and the nested lower levels
$\ell=1\subset\cdots\subset5$ are fully contained in each corpus, so no lower level
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:joint}%
    ...
    value-only sparse interpolant (``value'') against the
    value$+$gradient$+$cross (full-bilinear
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:polish}%
    ...
    $\mathcal{O}(1)$. \emph{Blue:} the value-only surrogate (``value'');
    \emph{orange:} the shipped gradient-enhanced surrogate
    (``value$+$gradient$+$cross''), both POD-compressed to the same
    ...
    Sec.~\ref{sec:polish}.
    \todo{stale box --- ``$b\in[2,7]$'' above is the superseded separation range;
    the production box is $b\in[3,10]\,M$ (see the marker in
    Sec.~\ref{sec:model}).}}
\end{figure*}
...
$\mathcal{E}=\{\chi^{A}_{y},\chi^{B}_{y}\}$, completed to full bilinear, is at or near
...
tangent) is $583$~MiB and the eight-dimensional one $13.5$~GiB. This is a
...
ranks the four-dimensional model shrinks by a factor of $51$ ($583$~MiB to
$11.5$~MiB) and the eight-dimensional one by $30$ ($13.5$~GiB to $461$~MiB), and
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:pod}%
    ...
    \emph{blue,} the value-only surrogate (``value''); \emph{orange,} the shipped
    gradient-enhanced one (``value$+$gradient$+$cross''), which stores the node
    ...
    shipped ranks the models shrink $51\times$ and $30\times$ at no cost to the
    certified output.}
\end{figure*}
...
Chebyshev$^2\times$Fourier grid, LM-initial-data with the prolate single patch of
...
For a misaligned-spin slice ($|\vect{S}^{A}|=0.3$ tilted $56.3^\circ$ off the collision
...
cross-checked against the closed form $J^i=\sum_X((S^{X})^{i}+(x_X\times P_X)^i)$ and
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:3dval}%
    ...
    \emph{Left:} for a misaligned-spin slice ($\vect{S}^{A}=(0.3,0,0.2)$, $b=1.5$,
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:tpval}%
    ...
    spin tilt exactly ($\vect{J}=\sum_X \vect{S}^{X}$): all magnitudes collapse onto
...
momentum is along $+y$; the proper rotation taking the LM-initial-data frame to
TwoPunctures maps our $y$ to TP-$z$, and ``aligned spin'' therefore means the
...
Finally, the closed-form angular momentum $J=2b\,P_t+S^{A}_{y}+S^{B}_{y}$ (along $+y$)
...
LM-initial-data solution, and our initial data is as evolution-ready as TwoPunctures'.
...
\begin{table}[t]
  ...
  \input{tables/tab01_tangent_operator.tex}
\end{table}
...
\begin{table}[t]
  ...
  \input{tables/tab02_tangent_surrogate.tex}
\end{table}
