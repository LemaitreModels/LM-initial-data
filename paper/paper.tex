...
production box (App.~\ref{sec:validation:qc}) and after interpolation onto
an evolution grid (App.~\ref{sec:validation:constraints}). In the
...
\begin{figure}[t]
  ...
  \caption{\label{fig:constraints}%
    \textbf{Constraint violation of the initial data on a uniform Cartesian
    evolution grid.} Bulk $L_2$ norms of the Hamiltonian and momentum
    constraints against the Cartesian grid spacing $h$, for the axisymmetric
    configuration of App.~\ref{sec:validation:qc} (equal bare masses,
    half-separation $b=3$, axial momentum $P=0.5$), on the four grids
    $N^3$ with $N=40$, $56$, $72$, $88$ across a box of half-width $9\,M$ with
    the punctures excluded to radius $1.5\,M$. Both norms are measured by the
    generic finite-difference constraint monitor described in the text, which
    makes no use of conformal flatness, and both fall at its second order
    (measured $2.14$ and $2.02$; dotted, $\propto h^{2}$). Black open markers
    repeat the measurement with the conformal factor supplied by TwoPunctures
    on the same grids; the two agree to better than $10^{-6}$ relative at every
    rung, so what is plotted is the truncation error of the Cartesian
    discretization rather than a property of either initial-data solution.}
\end{figure}
...
spacing $h$, as shown in Fig.~\ref{fig:constraints}, the measured orders being
$2.14$ for $\|\mathcal{H}\|_2$ and $2.02$
...
Cartesian grids (Fig.~\ref{fig:constraints}, open markers). The measured residuals are therefore set by the
