...
Sampling the spins in $\chi^{X}_{i}$ rather than in the bare-mass spin $S^{X}_{i}$
keeps the parameter domain rectangular, because the physical Kerr bound
$|\chi^{X}_{i}|\le1$ is independent of the component masses. Interpolating directly
in $S^{X}_{i}$ would instead make the admissible spin range vary with the mass
...
subspace --- the \emph{complete} tensor product there, node-exact for the value and
for \emph{every} first and mixed tangent. The cross term is as cheap as the first
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:peraxis}%
    ...
    Eq.~\eqref{eq:hermite} --- present in the joint models of
    Figs.~\ref{fig:joint}--\ref{fig:pod} --- does not enter
