...
The surrogate of Eq.~\eqref{eq:surrogate} then generalizes to
\begin{widetext}
\begin{equation}
...
interpolant, and it is the cross that recovers them and carries the surrogate past the
...
The gradient-enhanced surrogate is thus more accurate at no additional expensive cost: it
...
Certification is unaffected: the enhanced surrogate remains only a warm start, refined to tolerance by the certified
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:peraxis}%
    ...
    compared with the gradient-enhanced one, which also matches the certified
    parameter tangent at
    each node. A one-parameter sweep carries no mixed tangent, so the cross term of
    Eq.~\eqref{eq:hermite} does not enter
    here. Markers are medians over $100$
...
\begin{figure}[!t]
  ...
  \caption{\label{fig:walls}%
    ...
    coincidence while $b_\ast$ stays pinned at the merger --- a hard, real wall.
    \emph{Middle (mass ratio $q$):} the rate degrades only mildly and $q_\ast$
    recedes roughly in proportion to the interval, so the box never approaches it
    --- again a complex pair, hence soft.
    \emph{Bottom (spin $\chi$):} the rate degrades only mildly and $\chi_\ast$
    marches outward, beyond the sampled range --- a complex-conjugate pair off the
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:joint}%
    ...
    the best, median, and worst of the distribution; every level is served from
    the committed corpus at the node counts shown, with no new solves.
    \emph{Left (four-dimensional model, $(b,q,\chi^{A}_{y},\chi^{B}_{y})$):} the
    ...
    \emph{Right (eight-dimensional general-spin model):} the same two curves.
    At a given Smolyak level the sparse grid costs $\sim\!14\times$ the nodes and
    lands about two orders of magnitude less accurate, and because only two of the
    eight axes are enhanced, the gradient-enhanced model separates from the
    value-only one only from $\ell=2$ onward and by a smaller margin
    ($1.7\times$ in the median at $\ell=5$, against $4\times$ in four
    dimensions). The
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:polish}%
    ...
    $10^{-10}$. \emph{Grey:} a surrogate-free \emph{cold start}
    from the solver's zero-field iterate, whose bare residual is
    $\mathcal{O}(1)$. \emph{Blue:} the value-only surrogate;
    \emph{orange:} the shipped full-bilinear gradient-enhanced
    surrogate, both POD-compressed to the same
    ...
    three Newton steps in four dimensions and four in eight --- two to three in the
    median, against four to five for the cold start --- \emph{independent} of the
...
Eq.~\eqref{eq:hermite} that turns it into the surrogate shipped in
Fig.~\ref{fig:joint}.
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:pod}%
    ...
    $\star$ marks the full-rank model. Every panel carries the same two families:
    \emph{blue,} the value-only surrogate; \emph{orange,} the shipped
    gradient-enhanced one, which stores the node
    tangents and the bilinear cross tangent of Eq.~\eqref{eq:hermite} alongside the
    value and therefore sits at larger memory for the same rank.
    Columns share the memory axis:
    \emph{left,} the four-dimensional quasi-circular model; \emph{right,} the
    eight-dimensional general-spin model (Sec.~\ref{sec:model:8d}).
    Compressing raises the bare-guess residual, but
    ...
    shipped ranks the models shrink $49\times$ and $30\times$ at no cost to the
    certified output.}
\end{figure*}
...
\begin{figure}[t]
  ...
  \caption{\label{fig:targeting}%
    ...
    black-box loop takes a target-dependent median of ten. Measured on the earlier
...
We then test whether the gradient exposed by the surrogate reproduces this
sensitivity.
...
We next verify the sensitivity exposed by the surrogate.
...
central finite differences of the same surrogate to
...
of the surrogate recovers this tangent to the accuracy of the parametric
interpolation.
