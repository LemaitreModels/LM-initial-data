...
% ===========================================================================
\section{Numerical solution of the constraint equation}
\label{sec:solve}
% ===========================================================================
...
The regular correction is represented nodally: the unknown is the tensor of collocation values $U_{ijk}\equiv u(A_i,B_j,\phi_k)$, rather than the modal coefficients.
...
conditions, and is block-diagonal in the azimuthal mode $m$.
The subscript ``int'' indicates that the nonlinear source is applied only
on the interior rows.
The Bowen--York source couples the Fourier modes whenever the data are
non-axisymmetric. We therefore evaluate the nonlinear term
pseudo-spectrally on the $\phi$ collocation grid and solve the resulting
coupled Newton system with the exact Jacobian
Eq.~\eqref{eq:jacobian} in matrix-free form.
The iteration converges quadratically until the row-equilibrated residual
stagnates, at which point the best iterate is returned.


The structure of the Jacobian explains the organization of the matrix-free
solver. The linear operator $M_0$ is block-diagonal in the azimuthal Fourier
...
We solve the coupled Newton system with the full Jacobian
Eq.~\eqref{eq:jacobian} in matrix-free form. Only the matrix--vector product
$J\,\delta u$ is required. It is assembled exactly as the residual: the linear
...
linear operator factored once by its row-equilibrated LU decomposition. Since
this preconditioner differs from the full Jacobian only through the omission of
the Fourier-mode coupling, it provides an accurate approximation. Consequently,
only a few GMRES iterations are required, while the outer Newton iteration
retains quadratic convergence. This Newton--Krylov solve is the operation
invoked by the certified polish of Sec.~\ref{sec:polish}.


Finally, we specify the norm used to monitor convergence.
The prolate operator's rows span the $\sim\!10^{13}$--$10^{15}$ dynamic range of
Sec.~\ref{sec:solve:abt} (the $(1-A^2)^4$ factor tiny near infinity, the $1/A$
factor large on the inner axis). The \emph{raw} nodal residual $\|R\|_\infty$ is
therefore dominated by the badly-scaled rows near the inner axis: it floors well
above machine precision and even \emph{rises} with resolution. This is a scaling
artifact, not a failure to satisfy the
constraint. Row equilibration removes it: scaling each row by its factor $s^{(m)}_i \equiv \max_j \bigl|(M_0^{(m)})_{ij}\bigr|.$
(Sec.~\ref{sec:solve:abt}) is an exact rescaling of the equations that leaves the
solution unchanged but collapses the condition number, so the honest measure is the
\emph{row-equilibrated} residual $\max_m \max_i \big|\hat R^{(m)}_{i}/s^{(m)}_{i} \big|$. It is this
well-conditioned norm the equilibrated Newton iteration actually drives to zero.
...
\subsection{The parametric problem and sparse interpolation}
\label{sec:param:interp}

We consider the parameter vector $\theta \equiv (q, b, \boldsymbol{\chi}_A, \boldsymbol{\chi}_B)$ at fixed total
bare mass $M \equiv m_A + m_B$ (set to $1$), the spins carried as the Kerr-like
\emph{dimensionless} spins $\boldsymbol{\chi}_X \equiv \boldsymbol{S}_X/m_X^2$ (each a Cartesian 3-vector,
referred to the bare puncture mass $m_X$).
...
interpolation axes are $q$, $b$, and the individual dimensionless-spin
\emph{components} $\chi_{X,i}$, i.e.\ up to eight scalar axes for the full two-spin
family, fixed independent of
$q$ and symmetric about zero.

We parameterize the spins by the dimensionless quantities
$\chi_i \equiv S_i/m_i^2$ rather than the physical spin magnitudes $S_i$.
Using $\chi_i$ keeps the parameter domain rectangular and ensures that the
physical Kerr bound, $|\chi_i|\le1$, is independent of the component masses.
In contrast, interpolating directly in $S_i$ would make the admissible spin
range vary with the mass ratio, distorting the parameter space and reducing the
efficiency of tensor-product interpolation. Each interpolation axis is
subsequently mapped affinely onto $[-1,1]$, matching the natural domain of the
Chebyshev basis and promoting rapid spectral convergence.
...
For any queried $\theta$ it exposes the parameter sensitivities
$\partial U/\partial\theta$ of the emitted field and, through any observable $F$
constructed from it, the sensitivity $\partial F/\partial\theta$. These are
derivatives of the certified initial data.

That the interpolant is differentiable is unremarkable; the content of the claim is
that the exposed derivative is the sensitivity of the \emph{certified} solution.
The scientific challenge is therefore not differentiating the interpolant, but preserving differentiability through the projection back onto the Einstein constraint manifold.
The interpolant of Eq.~\eqref{eq:surrogate} serves only as a warm start, and the emitted
datum is the constraint-satisfying field produced from it by the Newton refinement of
Sec.~\ref{sec:polish}. Implemented in JAX~\cite{Jax2018} as a branchless barycentric
map, the surrogate admits forward-mode automatic differentiation, which returns
$\partial U/\partial\theta$ and $\partial F/\partial\theta$ directly, without finite
differences or a hand-written adjoint.

{\color{red}
That these sensitivities are those of the certified solution, and not of the
interpolant alone, is established by three independent checks. They agree with
second-order central finite differences to a relative accuracy of
$2.96\times10^{-10}$. They agree with the implicit-function-theorem tangent of the
\emph{certified} solve, $J\,(dU/d\theta) = -\,\partial R/\partial\theta$, to
$1.7\times10^{-7}$ along the spin axes and $\sim1.3\times10^{-5}$ along $b$ and $q$
(interpolation-accuracy-limited, improving exponentially with the parameter order).
The spin tangent requires a dedicated analytic source derivative: $\chi$ enters the
...
which matches autodiff of the source to $\le 10^{-12}$. Finite differences, the
certified tangent solve, and the closed-form source derivative therefore agree, so
the exposed gradient is the sensitivity of the certified initial data, not the
derivative of an interpolant of itself.}

This is what separates the exposed sensitivities from those of a bare interpolant or
a neural-network surrogate: the emitted fields have already been projected back onto
the Einstein constraint manifold by certified Newton refinement, so their derivatives
are meaningful sensitivities of constraint-satisfying initial data. Certification and
differentiability coexist rather than compete --- every returned datum satisfies the
constraints to tolerance, and its exact dependence on the physical parameters is
available at once. These verified derivatives are what make the gradient-based
parameter targeting and effective-potential eccentricity reduction of
Sec.~\ref{sec:applications} possible.
...
$U_i'\equiv dU/d\theta_k|_{\theta_i}$, produced during the offline build for the cost
...
left approximated, lowering the joint held-out error (Fig.~\ref{fig:joint}). For more
...
{\color{red}Figure~\ref{fig:polish} makes this quantitative for both shipped
quasi-circular models ($\chi$, $b\in[2,7]$, Smolyak $\ell=5$; the four-dimensional
aligned-spin and eight-dimensional general-spin models of
Sec.~\ref{sec:model:joint}).
Over $1000$ uniformly-random off-node points the bare sparse-grid guess is often
poor, e.g.\ at the hard corners of the box (small separation, unequal mass,
opposite-sign spins) it reaches $\|R\|_\infty$ up to $\sim\!1$, yet the Newton
polish converges quadratically and certifies \emph{every} one of the $1000$ points
to $\|R\|_\infty\le10^{-10}$ in at most four steps, typically two (four-dimensional)
or three (eight-dimensional) --- about two steps ahead of a surrogate-free cold
start --- \emph{independent} of how far from converged the interpolant is.}

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{fig01_polish_staircase.pdf}
  \caption{\label{fig:polish}%
    \textbf{Certified refinement.} \emph{Top row:} constraint residual
    $\|R\|_\infty$ over $1000$
    uniformly-random off-node points of the quasi-circular models ($\chi$,
    $b\in[2,7]$, Smolyak $\ell=5$; left column: the four-dimensional aligned-spin
    model, right column: the eight-dimensional general-spin model), before the
    polish (``guess'') and after each Newton step; the two columns share the
    Newton-step axis. Each curve is the median (line) with
    min--max whiskers over the $1000$ points. \emph{Grey:} a surrogate-free
    \emph{cold start} from the solver's zero-field iterate, whose bare residual is
    $\mathcal{O}(1)$ (up to $\sim\!1$ at the hard corners of the box --- small
    separation, unequal mass, opposite-sign spins) and which needs four to five
    steps. \emph{Blue:} the value-only surrogate; \emph{orange:} the shipped
    gradient-enhanced (Hermite) surrogate --- both POD-compressed to the same rank
    ($r=75$ in 4D, $r=250$ in 8D), at which the median bare guess is not visibly
    changed, so the two curves are compared at equal spatial rank and differ only
    in the gradient enhancement. Both surrogate warm starts certify
    \emph{every} point to $\|R\|_\infty\le10^{-10}$ in at most four steps ---
    median two (4D) or three (8D), about two steps ahead of the cold start ---
    \emph{independent} of the interpolation error. \emph{Bottom row:} the
    corresponding relative field error $\|u-u_{\rm true}\|_2/\|u_{\rm true}\|_2$
    per step for both columns, measured against the converged certified iterate.
    Both surrogate warm starts are already field-accurate at the guess
    (gradient-enhanced $\sim\!2\times10^{-5}$ in 4D and $\sim\!10^{-3}$ in 8D,
    the value-only surrogate a factor $\sim\!2$ behind) even though the residual
    there is $\mathcal{O}(10^{-2})$ (4D) to $\mathcal{O}(10^{-1})$ (8D) --- the
    decoupling discussed below --- and both fall to the numerical floor within a
    few steps, while the cold start trails by about two.}
\end{figure*}

At this point, it is important to note that two quite different quantities measure how good a guess
$u^{(0)}$ is: its \emph{field} error $\|u^{(0)}-u_{\rm true}\|_2/\|u_{\rm true}\|_2$, i.e.\ the ordinary distance between fields, which the interpolation accuracy controls, and the \emph{constraint residual}
$\|R(u^{(0)})\|_\infty$, which is what the Newton iteration~\eqref{eq:polish}
actually reduces. These are nearly \emph{decoupled}. The residual operator
differentiates the field (it contains the Laplacian, at scale $\propto b^{-2}$),
so it amplifies precisely the small-amplitude, high-derivative content that the
field norm barely registers: a guess can be excellent in field-norm yet violate the
constraint by orders of magnitude more. Concretely, projecting the \emph{exact}
solution onto a smooth low-dimensional subspace leaves a field error
$\sim\!10^{-4}$ but a residual $\|R\|_\infty\sim\mathcal{O}(1)$, and it certifies
in no fewer Newton steps than the crude barycentric guess. The step count is
therefore set by $\|R(u^{(0)})\|_\infty$, not by field accuracy, with two
consequences. First, refining the interpolant's field accuracy (finer grids,
richer bases) is a weak lever on the online cost, and the certified polish,
which drives $\|R\|\to0$ directly regardless of the guess, is the correct
instrument. Second, one may discard the high-derivative content of
the stored fields with no penalty, because the polish reconstructs it; this makes reduced-basis re-encoding interesting.
...
artifact, with the value-only surrogate its $\mathcal{E}=\varnothing$ baseline. The
accuracy metric throughout is the \emph{held-out}
interpolation error: at a parameter point that is \emph{not} a collocation node we
compare the interpolant to a direct elliptic solve on the \emph{same} frozen
spatial grid ($N_A=44$, $N_B=32$, $N_\phi=8$), so the spatial discretization error
cancels and what remains is the pure parameter-interpolation error ($21$
held-out points per one-axis sweep; $1000$ uniform random off-node points for the
joint errors).
...
Although the particular choice of momentum condition is essential for the physics of the quasi-circular family,
it is not essential for the interpolation machinery described in this paper and therefore can readily be replaced by any other formulation if desired.


\subsection{Convergence across the parameter space}
\label{sec:model:peraxis}
Sweeping one parameter at a time (the other seven averaged over $100$ random
interior base points), every axis converges geometrically (Fig.~\ref{fig:peraxis}). The
separation $b$ and mass ratio $q$ are fast ($0.52$
and $0.60$ decades/node); the spins are slower but, crucially, \emph{equalize}
across the two holes once sampled in the dimensionless $\chi$ rather than the
bare-mass $S$: the aligned axes converge at $0.46$ ($\chi_{Ay}$) and $0.46$
($\chi_{By}$) decades/node and the in-plane axes at $\approx0.46$--$0.47$, all
within a narrow band. This is the rectangle-versus-fan gain of
Sec.~\ref{sec:param:interp} made quantitative: in the bare-mass spin $S$ the same
astrophysical box drives the small hole toward extremal $\chi$, and its aligned
axis is $\sim\!2.6\times$ slower than the large hole's; sampling in $\chi$ adapts
the node density to each hole's mass and removes that penalty, at only a marginal
cost to the large-hole axis. Gradient enhancement
(Sec.~\ref{sec:param:hermite}) roughly doubles every rate --- $b\to1.01$,
$q\to1.18$, and the spins to $\approx0.88$--$0.90$ decades/node --- for the price
of one extra back-solve per node against the node's already-factored Jacobian. The
joint held-out error is set by these per-axis rates
(Fig.~\ref{fig:joint}), the slowest axis dominating.
...
$|\xi^\ast|=\tfrac12(\varrho+\varrho^{-1})$, and map $\xi^\ast$ back through the affine map
--- taking the real root outside the sampled box on the physically singular side
...
Both wall sweeps are run at equal mass ($q=1$, the other parameters at their
symmetric values), so along the spin axis the dimensionless spin is
$\chi=S_y/m^2=4\,S_y$.
The \emph{merger} wall, in the separation $b$, is a hard wall at the edge of
parameter space. As the sampling interval is pushed toward coincidence the rate
degrades smoothly ($0.74\to0.43$~decades/node as $b_{\min}=2.0\to1.0$, at
$b_{\max}=4$) and tracks the Bernstein prediction for a singularity fixed at
$b=0$, the inferred nearest real singularity staying at $b^\ast\approx0$
throughout. This resolves a concern
specific to the orbiting family: the $b\to0$ limit is \emph{doubly} singular --- the
punctures coincide \emph{and} the post-Newtonian tangential momentum
$P_t\propto b^{-1/2}$ diverges --- yet the extra branch does not \emph{relocate} the
wall off $b=0$; its only signature is a mild ($\sim\!10\%$) undershoot of the measured
rate below the pure-coincidence prediction at the smallest $b_{\min}$, i.e.\ the
momentum divergence makes the wall marginally \emph{stronger}, not closer.

The \emph{spin} wall, in $\chi$, is by contrast soft. The measured rate degrades
only mildly as the range grows ($\approx0.46\to0.37$ over
$\chi_{\max}=1.6\to2.4\to3.2$), and the inferred nearest \emph{real}
singularity marches outward ($\chi^\ast\approx2.1\to3.0\to3.8$; here the sweep
deliberately extends past the production box $|\chi|\le0.99$ to locate the wall),
staying beyond the sampled range throughout
(Fig.~\ref{fig:walls}, right) --- the signature of a
complex-conjugate singularity pair off the real axis rather than a real wall. The
nearest feature sits at $\chi^\ast\gtrsim2$, already super-extremal (well past the
Cook--York horizon-spin ceiling~\cite{CookYork1990}), so spin is a benign axis with
no sharp singularity anywhere near the physical box.

It is worth drawing out a comparison that runs against intuition. What sets the
convergence rate is not the \emph{hardness} of a wall --- whether the nearest
singularity is a real branch point or a complex-conjugate pair --- but its
\emph{distance} from the sampling interval. The separation $b$ carries the only
\emph{hard} (real) wall of the family, yet it is the \emph{fastest}-converging
axis (Fig.~\ref{fig:peraxis}), because that singularity sits at $b^\ast\approx0$,
far below the sampled box. The aligned spins, by contrast, have only \emph{soft}
(complex, receding) walls and are nonetheless the \emph{slowest} axes, because in
the physical spin variable $S_y$ the small hole's dimensionless spin
$\chi=S_y/m^2$ drives the sampling interval up against its (far, but effectively
near) singularity. A hard wall far away costs less than a soft wall close by, and
it is the small-hole spin --- not the merger separation --- that dominates the
offline solve count.
...
four-dimensional model is \textbf{$1105$ solver nodes} and the eight-dimensional
one \textbf{$15{,}713$} (field shape $45\times32\times8$); the nested lower levels
$\ell=1\subset\cdots\subset5$ are fully contained in each corpus (no additional
solves), and a bare query is a sub-second contraction over the stored solves,
dropping to milliseconds after the reduced-basis re-encoding of
Sec.~\ref{sec:model:pod}. The offline saving is decisive: a tensor grid built,
from the measured per-axis
rates, to \emph{bare} held-out accuracy $10^{-6}$--$10^{-9}$ would cost
$10^{2}$--$10^{4}\times$ more solves --- an accuracy the certified construction
never needs, because the sparse grid already supplies a Newton-basin warm start
everywhere in the box and the polish carries the correctness (the residual/field
decoupling of Sec.~\ref{sec:polish}). Even at matched per-axis resolution the
comparison is lopsided: the full tensor grid on the four-dimensional sparse grid's
finest level ($33$ nodes per axis) would be $33^4\approx1.2\times10^6$ solves,
$\sim\!10^3\times$ the sparse count. Because the model is nothing but its stored
nodal solves, its memory is \emph{linear} in the node count.
...
is the reduced step count --- two to four polish steps rather than the $\sim\!7$
a cold start needs. The practical pattern is thus to bulk-generate initial data
with bare queries and certify only the configurations one actually evolves.

A single distinction frames each model correctly: the \emph{bare} interpolant
is a warm start, not the product. For the four-dimensional model the joint held-out
error over $1000$ random off-node points falls geometrically with the Smolyak level
--- worst
$6.9\times10^{-2}\to1.3\times10^{-2}\to2.1\times10^{-3}\to3.1\times10^{-4}\to
4.6\times10^{-5}$ and median
$2.1\times10^{-2}\to3.3\times10^{-3}\to5.0\times10^{-4}\to7.2\times10^{-5}\to
1.0\times10^{-5}$ over $\ell=1$--$5$, the shipped $\ell=5$ reaching a worst
$\sim\!5\times10^{-5}$ and a best $1.7\times10^{-7}$ (Fig.~\ref{fig:joint}); every
level is served from the committed corpus with no new solves. The certified polish
then makes even that warm-start accuracy \emph{irrelevant to correctness}: a few
Newton steps of the genuine solver drive the constraint residual to
$\|R\|_\infty\le10^{-10}$ at any queried parameter (Fig.~\ref{fig:polish}),
\emph{independent} of the interpolation error. The interpolant accelerates; the
polish certifies.
...
whether $\mathcal{E}$ should be enlarged beyond the two aligned-spin axes. Extending it
to all four axes of the aligned-spin box, $\mathcal{E}=\{b,q,\chi_{Ay},\chi_{By}\}$, at
the same level and box ($\ell=5$, $1105$ nodes), degrades the surrogate far below doing
nothing at all: the held-out median over $1000$ certified reference solves rises from
$9.97\times10^{-6}$ (value-only) to $1.01\times10^{-3}$ for the gradient-only four-axis
model, and the pairwise-bilinear completion does not repair it ($4.1\times10^{-3}$, and
non-monotone in level). We traced this carefully, because a first attempt conflated it
with a genuine bug: in the dimensionless-spin parameterization $S_X=\chi_X m_X^2$, so
$dU/dq$ at fixed $\chi$ must carry the mass-to-spin chain
$dS_X/dq=\chi_X\,d(m_X^2)/dq$; omitting it makes the $q$ tangent $\sim\!9\times$ wrong
at $\chi\neq0$ (and exactly right at $\chi=0$). Restoring it improves the four-axis
model by $3.7\times$, but leaves it $101\times$ worse than value-only. Nor is the loss
due to the reduced-basis compression of Sec.~\ref{sec:model:pod} (compressed and
uncompressed models agree to four digits), to the cross layer, or to the
one-dimensional Hermite operators themselves:
sweeping each axis down to the three-node factor that the sparsest subgrids actually
use, Hermite still beats Lagrange on every axis, by $8\times$ ($b$), $16\times$ ($q$)
and $3.4\times$ ($\chi_{Ay}$). What does correlate is the \emph{number} of
simultaneously enhanced axes: enhancing any single axis improves on value-only
($0.34\times$ for $b$, $0.85\times$ for $q$), both two-axis sets we tested degrade
($4.7\times$ and $5.6\times$), and the four-axis set degrades by $101\times$. This is
consistent with cancellation in the Smolyak combination being governed by the
one-dimensional operator norms, which multiply over the enhanced dimensions while the
accuracy gain enters only once per axis; we have not established that mechanism
directly. The practical consequence is that $\mathcal{E}$ should be kept small and
spent on the slowest axes, and that the shipped $\mathcal{E}=\{\chi_{Ay},\chi_{By}\}$,
completed to full bilinear, is at or near the optimum of the configurations we
measured.

The eight-dimensional general-spin model, built by the same machinery, is
reported in Sec.~\ref{sec:model:8d}.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{fig02_peraxis_hermite.pdf}
  \caption{\label{fig:peraxis}%
    \textbf{Per-axis convergence of the quasi-circular models.}
    Held-out interpolation error vs.\ the number of parameter nodes for each of the
    eight axes $(b,q,\boldsymbol{\chi}_A,\boldsymbol{\chi}_B)$ --- the first four
    are the four-dimensional aligned-spin model --- comparing the value-only
    surrogate with the gradient-enhanced (Hermite) one that also matches the
    certified parameter tangent at each node. Each marker is the median over $100$
    random base points (the seven non-swept axes drawn uniformly in their boxes),
    with whiskers spanning the min--max (best--worst) range across those points.
    All decay geometrically (rates fit from the median curves;
    Sec.~\ref{sec:model:peraxis}), and matching the tangent roughly doubles every
    rate.
    Sampled in the dimensionless spin $\chi$ the two holes' spin axes
    \emph{equalize} ($\approx0.46$ decades/node), removing the small-hole
    penalty the bare-mass spin $S$ incurs (Sec.~\ref{sec:model:peraxis}); the spin
    analyticity wall sits at $\chi^\ast\gtrsim2$, far outside the box
    (Fig.~\ref{fig:walls}).}
\end{figure*}

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{fig03_walls.pdf}
  \caption{\label{fig:walls}%
    \textbf{The two analyticity walls --- separation (hard, real) vs.\ spin (soft, complex).}
    Held-out interpolation error vs.\ the number of nodes along each axis, for a range of
    sampling intervals, with the geometric fit $\varepsilon\approx A\,10^{-\rho Q}$ overlaid
    (dashed). Each fit, and hence its dashed line, spans only the clean geometric window ---
    the trailing nodes at which the held-out error has saturated on the $\sim\!10^{-11}$
    float64 floor are excluded from the fit, so a line stops short of the full node range
    wherever the error has already floored. Each legend entry gives, for
    that sampling range, the fitted rate $\rho$ (decades/node) and the inferred nearest real
    singularity $\theta^\ast$ --- whether $\theta^\ast$ holds still or marches outward as the
    range grows is what separates the two wall types.
    \emph{Left (merger wall, in the separation $b$):} the rate degrades as the interval is
    pushed toward coincidence ($0.74\to0.43$~decades/node as $b_{\min}=2.0\to1.0$) while the
    inferred singularity stays pinned near the merger ($b^\ast\approx0$, far below every
    $b_{\min}$) --- a hard real singularity fixed at $b=0$; the quasi-circular
    $P_t\propto b^{-1/2}$ divergence strengthens the wall but does not move it off $b=0$.
    \emph{Right (spin wall, in $\chi$):} the rate degrades only mildly ($0.46\to0.37$) and
    $\chi^\ast$ marches outward ($\approx2.1\to3.0\to3.8$), staying beyond the sampled range
    --- the signature of a complex-conjugate pair off the real axis, so spin is a benign axis
    with no sharp feature near the physical box.}
\end{figure*}

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{fig04_joint_dist.pdf}
  \caption{\label{fig:joint}%
    \textbf{Joint held-out convergence.}
    Joint held-out error over $1000$ random off-node points across Smolyak levels
    $\ell=1$--$5$, shown as the best, median, and worst of the distribution
    (whiskers span min--max about the median), every level served from the
    committed corpus at identical node counts (no new solves).
    \emph{Left (four-dimensional model, $(b,q,\chi_{Ay},\chi_{By})$):} the bare
    sparse interpolant (worst $6.9\times10^{-2}\!\to\!4.6\times10^{-5}$, median
    $2.1\times10^{-2}\!\to\!1.0\times10^{-5}$, best $1.7\times10^{-7}$ at
    $\ell=5$) against the value$+$gradient$+$cross (full-bilinear Hermite--Smolyak)
    model enhanced on the two spin axes (worst
    $6.8\times10^{-2}\!\to\!1.8\times10^{-5}$, median
    $1.2\times10^{-2}\!\to\!2.8\times10^{-6}$, best $4.7\times10^{-9}$ at
    $\ell=5$) --- a lower error at every level and a $\sim\!35\times$ deeper
    best-case floor at the same node count, from the node tangents alone.
...
like the aligned ones (Fig.~\ref{fig:peraxis}):
$\approx0.44$--$0.45$ decades/node in value and $\approx0.84$--$0.88$
gradient-enhanced --- the same cross-hole equalization that $\chi$-sampling buys in
the aligned model --- and the in-plane spin wall is again soft, its nearest real
singularity receding from $\chi^\ast\approx2.2$ to $4.1$ as the fit range grows, so
precession introduces no new hard wall. Every certified spot-check reaches
$\|R\|_\infty\le1.3\times10^{-11}$, though at this dimension it is the polish, not
the bare sparse interpolant, that carries the accuracy; the full $1000$-point
certified-polish statistics for both $\chi$ $\ell=5$ models are reported in
Fig.~\ref{fig:polish}. The $12.1$~GiB gradient-enhanced corpus reduced-basis compresses
to $461$~MiB with no loss of certification or differentiability
(Sec.~\ref{sec:model:pod}).

\subsection{Reduced-basis compression of the model}
\label{sec:model:pod}

Because the model \emph{is} its stored nodal solves, its memory is linear in the
node count (Sec.~\ref{sec:model:joint}): the gradient-enhanced four-dimensional
...
reconstruction error of the corpus (relative $L_2$) falls below $10^{-4}$ by
$r=33$ modes and below $10^{-6}$ by $r=87$; because the value and its parameter
tangents \emph{share} one spatial basis, the enhanced corpus needs only
$\sim\!12\%$ more modes than the value-only one at the same tail. The rank at a
fixed accuracy is set by the intrinsic dimension of the solution manifold, not by
the sampling density: it is the counterpart to Sec.~\ref{sec:param:smolyak} along
the orthogonal axis --- the Smolyak level fixes the parameter resolution and the
offline solve count, POD the spatial rank, and the two are independent.

Figure~\ref{fig:pod} shows the memory--accuracy tradeoff. At the shipped rank the four-dimensional
model shrinks from $486$~MiB to $11.5$~MiB ($42\times$; $r=87$ at the $10^{-6}$
tail, $132\times$ per field, preserving the parameter gradient to $\sim\!10^{-6}$),
and its per-query evaluation, now a barycentric sum over $r$ rather than $11{,}520$
coefficients and JIT-compiled, drops to a few milliseconds;
the eight-dimensional model compresses from $12.1$~GiB to $461$~MiB ($27\times$;
$r=394$ at the $10^{-6}$ tail, $29\times$ per field). The certified constraint
residual, the number of Newton steps to reach it (Sec.~\ref{sec:polish}), and the
verified $\partial\text{ID}/\partial\theta$ (Sec.~\ref{sec:param:diff}) are all
preserved: the mode projection loses only the truncation-tail fraction of the
gradient, the compressed model reproduces the truncated interpolant to that tail
and reloads as a standalone predictor. The decomposition is computed once from the
existing corpus (minutes), with no new solves.

Three further compressions stack on top, all free (computed from the same corpus,
no new solves) and all preserving the certified Newton-step count and the parameter
gradient: single-precision storage of the spatial modes (halving the mode storage,
the gradient preserved to $\sim\!3\times10^{-8}$), an anisotropic Smolyak grid
weighted toward the slowly-converging spin axes ($\approx\!3\times$ fewer nodes at
the same worst-case Newton-step count), and a per-Fourier-mode basis that stores the
dominant ($99.97\%$-energy) $m=0$ block in its compact $1440$-value form rather than
the $8\times$-redundant nodal form ($\approx\!5\times$ on the mode storage).
Together they reduce even the eight-dimensional model several-fold further.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{fig05_guess_vs_memory.pdf}
  \caption{\label{fig:pod}%
    \textbf{Reduced-basis (POD) compression versus stored memory: bare-guess
    constraint residual (top row) and field error (bottom row).} Median line with
    min--max whiskers over uniformly-random off-node query points as the POD
    truncation rank $r$ is swept (each point labelled by $r$; $\star$ marks the
    full-rank, un-compressed model). Columns share the memory axis: \emph{left,}
    the four-dimensional quasi-circular model; \emph{right,} the eight-dimensional
    general-spin model (Sec.~\ref{sec:model:8d}). \emph{Top row:} the constraint
    residual $\|R\|_\infty$ of the bare barycentric guess. \emph{Bottom row:} the
    relative field error $\|u-u_{\rm true}\|_2/\|u_{\rm true}\|_2$ of the \emph{same}
    guess against the certified solve (four-dimensional model; same value and
    value$+$gradient [full-bilinear cross] corpora as the top row; the
    eight-dimensional field-error sweep is pending). The
    value-only and gradient-enhanced corpora compress at nearly the same rank, since
    they share one spatial basis. Compressing raises the bare-guess residual, but
    ...
    same Newton-step count regardless. The field error (bottom) meanwhile
    \emph{saturates} at its floor ($\sim\!8\times10^{-5}$ value,
    $\sim\!2\times10^{-5}$ value$+$gradient) at a far lower rank than the residual,
    so the redundancy discarded by compression is exactly the high-derivative
    content the residual amplifies and the field norm cannot see (the decoupling
    discussed after Fig.~\ref{fig:polish}). The shipped ranks ($r=87$ in 4D, $r=394$ in 8D;
    $10^{-6}$ singular-value tail) thus shrink the model $42\times$ and $27\times$
    ($486$~MiB${\to}11.5$~MiB and $12.1$~GiB${\to}461$~MiB) at no cost to the
    certified output.}
\end{figure*}
...
most demanding, genuinely non-axisymmetric configurations: the conformal factor
converges to $\max|\psi-\psi_{\rm TP}|\sim10^{-7}$ (resolution-limited), the total
ADM mass to $\sim\!10^{-12}$, and the ADM angular momentum exactly, with a
per-slice certified residual $\|R\|_\infty\lesssim10^{-11}$. The interpolated data
are moreover evolution-ready: their finite-difference constraint violation
converges at second order and is indistinguishable from TwoPunctures-sourced data.
Because certification is an \emph{internal} guarantee (Sec.~\ref{sec:polish}),
this external agreement is credibility, not the correctness claim itself; the full
comparison --- non-axisymmetric fields, the three-dimensional ADM angular
momentum, the quasi-circular data, and evolution-readiness --- is given in
Appendix~\ref{sec:validation}.
...
black-box variants take a median of ten (ranging $8$--$12$ with the target).
Warm-starting cuts the per-solve cost (median wall time $27\to18$~s) but \emph{not}
...
\begin{figure}[t]
  ...
  \caption{\label{fig:targeting}%
    \textbf{Certified parameter targeting.} Reaching a
    physical target $(\Madm,J)$ by adjusting $(b,q)$, over $100$ random
    known-answer targets, measured in \emph{certified} elliptic solves. Each method
    is drawn as the across-target median residual at each solve count, with whiskers
    spanning the minimum and maximum over targets. The differentiable method (using the parameter gradient on the free surrogate) reaches tolerance in at most three certified solves for
    every target, whereas the black-box control loop takes a target-dependent
    median of ten.}
\end{figure}
...
circular orbit on the free interpolant and certifies at the end. Sweeping
$J\in\{1.00,1.05,1.10\}$ traces the circular-orbit sequence, whose minimum shifts
outward with angular momentum ($b_{\rm circ}=3.43,\,4.94,\,6.04\,M$;
Fig.~\ref{fig:ecc}). The gradient locates each $b_{\rm circ}$ in three certified
solves against the scan's thirteen, with a certified
$\partial E_b/\partial b\approx3\times10^{-7}$ at the minimum; at $J=1.05$ the two
agree on $b_{\rm circ}$ to $5\times10^{-3}$. The gradient is moreover \emph{more
accurate} in the shallow, near-innermost-stable-orbit regime: at $J=1.00$ the fixed
thirteen-point scan mislocates the minimum by $\sim\!0.5\,M$, whereas the
derivative-based gradient still resolves it, so a black-box scan must sample ever
more finely, at more certified solves, exactly where the physics is most delicate.
The same differentiable $E_b$ reads the orbital eccentricity off the turning
points --- for the well-centred $J=1.05$, from $e=0$ at the circular orbit to
$e\approx0.23$ at the box edge --- exposing $\partial e/\partial\theta$ for
gradient-based e-reduction.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig07_eccentricity.pdf}
  \caption{\label{fig:ecc}%
    \textbf{Accelerated eccentricity reduction.} At fixed angular momentum $J$ the field-dependent binding energy
    $E_b(b)$ has a minimum at the circular orbit; sweeping $J\in\{1.00,1.05,1.10\}$
    the minimum shifts outward, tracing the circular-orbit sequence. For each $J$
    the differentiable $\partial E_b/\partial b$ locates the minimum in three
    certified solves (stars), against the classical certified-solve scan's thirteen
    (squares). In the shallow small-$J$ well the fixed scan mislocates the minimum
    (at $J=1.00$ by $\sim\!0.5\,M$), whereas the derivative still resolves it. Every
    emitted configuration is certified to $\|R\|_\infty\le10^{-10}$.}
\end{figure}
...
machinery is \emph{dimension-agnostic} --- it sees only a
...
Three accuracy caveats are worth stating explicitly. First, the two-centre (and
spinning) spatial floor sits above the single-centre $10^{-12}$ --- the steeper,
denser-coupled source carries more high-frequency content --- but this is
immaterial to the certified output, because the certified polish recovers
$\|R\|_\infty\le10^{-10}$ at any parameter regardless of where the bare interpolant
floors. Second, the spin-wall analysis is quantitative only within the physical
range ($\chi_{\max}\lesssim0.8$): pushing the sampling interval toward and past
extremal spin mixes the genuine (complex, far) singularity with pre-asymptotic and
spatial-floor effects, so the clean single-$\chi^\ast$ characterization is reported
for the physical regime, not extrapolated beyond it. Third, the certified
$\|R\|_\infty\le10^{-10}$ gate is verified at the production grid
($44\times32\times8$); at higher meridian resolution the whole-field equilibrated
residual floors higher (e.g.\ $\sim\!5\times10^{-10}$ at $N_A=56$), because the
stiff $m^2/\rho^2$ term amplifies roundoff in the azimuthal modes that carry no
source content ($\sim\!10^{-16}$), while the physically populated modes
($m\le2$) stay at $\sim\!10^{-13}$ --- the floor is set by empty modes, not by
the physics, and the certification tolerance should be read at the grid it is
quoted for.
...
a role that motivates the certified-and-differentiable design as much as the
immediate control-loop and targeting applications do.
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:3dval}%
    \textbf{First non-axisymmetric data, validated against TwoPunctures.}
    ...
    resolution ladder; the certified Newton--Krylov residual $\|R\|_\infty$ (solid) is
    three-to-five orders below the earlier block-diagonal monitor and near machine
    precision at the production resolution, its slow rise at higher meridian
    resolution being the empty-high-$m$-mode roundoff amplification of
    Sec.~\ref{sec:conclusions} (a norm artifact, not a solver limitation) --- the
    field keeps converging throughout.
    \emph{Right:} the azimuthal $\phi$-mode spectrum of the solved field decays
    exponentially in $m$; aligned spin ($\theta_S=0$) leaves every $m\!\ge\!1$ mode
    at machine zero, confirming that few Fourier modes resolve a minimal break of
    axisymmetry.}
\end{figure*}
...
\begin{figure*}[t]
  ...
  \caption{\label{fig:tpval}%
    \textbf{TwoPunctures validation: the 3-D observable and the quasi-circular data.}
    ...
    $\theta_J=\theta_S$, with TwoPunctures anchors (stars) on the line --- the
    genuinely three-dimensional observable the quasi-circular family relies on.
    \emph{(b,c)} On a quasi-circular slice ($b=4$, $q=1$, no spin) the conformal
    factor converges spectrally to TwoPunctures ($\max|\psi-\psi_{\rm TP}|\to
    1.7\times10^{-9}$ at $64\times44$, the two-centre spatial floor) and the total
    ADM mass converges far deeper still ($|M_{\rm ADM}-E_{\rm TP}|/E_{\rm TP}\to
    5.8\times10^{-12}$), the boundary-spectral $M_{\rm ADM}$ outrunning the
    pointwise field.}
\end{figure*}
